...
export function createMedicalRoom(scene) {
  ...
  // The arms, out of the wall either side of the television and not attached to
  // it. They belong to the character rather than to this room, so they are
  // built by it — and they are built *here*, after the collider list exists,
  // because each conduit puts a chain of boxes into it as it is made.
  const wallArms = createWallArms({
    parent: group,
    colliders,
    origin: new THREE.Vector3(cx, 0, cz),
    wallOffset: wallZ + 0.12,
  });
  ...
  // How far back into the wall the arms have got is wallArms' own business now.
  let shutDown = false;
  let dimmed = 0;
  ...
  return {
    ...
    get shutdownProgress() {
      return { dimmed: +dimmed.toFixed(2), retracted: wallArms.gesture.retracted };
    },
    ...
    /** Dev handles: the arms, which belong to wallArms.js now. */
    armSamples() {
      return wallArms.samples();
    },
    get gesture() {
      return wallArms.gesture;
    },
    stopSpeaking() {
      life.stop();
    },
    ...
    update(delta) {
      ...
      // The arms, which are the character's and not the room's: it gestures
      // while it talks, folds them on the lines that call for it, and takes
      // them back into the wall when it goes dark.
